...
@app.route("/api/programas/cargar_excel", methods=["POST"])
def api_cargar_programas_excel():
    """Cargar programas académicos desde un archivo Excel"""
    if 'usuario' not in session:
        return {"success": False, "error": "No autorizado"}, 401
    ...
    if not file.filename.endswith(('.xlsx', '.xls')):
        return {"success": False, "error": "El archivo debe ser un Excel (.xlsx o .xls)"}, 400
    ...
    try:
        ...
        if 'nombre' not in df.columns and 'Nombre' not in df.columns and 'NOMBRE' not in df.columns:
            return {
                "success": False, 
                "error": "El archivo debe tener una columna llamada 'nombre', 'Nombre' o 'NOMBRE'"
            }, 400
        ...
        df = df[df['nombre'] != '']  # Eliminar filas vacías
        df = df[df['nombre'] != 'nan']  # Eliminar NaN
        ...
        with get_db_connection() as conn:
            ...
            for index, row in df.iterrows():
                ...
                try:
                    ...
                    query_check = adapt_query("SELECT id FROM programas WHERE nombre = ?")
                    ...
                    if USE_POSTGRES:
                        cursor.execute("""
                            INSERT INTO programas (nombre, activo, fecha_creacion, fecha_modificacion)
        ...
        mensaje = f"Se agregaron {programas_agregados} programas exitosamente."
        if programas_duplicados > 0:
            mensaje += f" {programas_duplicados} programas ya existían y fueron omitidos."
        if errores:
        ...
        return {
            "success": True,
            "message": mensaje,
            "agregados": programas_agregados,
            "duplicados": programas_duplicados,
            "errores": errores
        }
    
    except Exception as e:
        return {"success": False, "error": f"Error al procesar el archivo: {str(e)}"}, 500
