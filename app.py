...
def asistencia_to_tuple(asistencia):
    ...
    if isinstance(asistencia, dict):
        ...
        return (
            ...
            asistencia.get('sede', ''),
            asistencia.get('fecha_evento', ''),
            asistencia.get('fecha_registro', '')
        )
    else:
    ...
    for programa in programas:
        if isinstance(programa, dict):
            ...
            result.append((
                ...
                programa.get('fecha_creacion', ''),
                programa.get('fecha_modificacion', '')
            ))
        else:
            # SQLite - ya es tupla o Row compatible
    ...
    if request.method == "POST":
        try:
            ...
            with get_db_connection() as conn:
                ...
                """)
                cursor.execute(query, (asistencia_id, evaluacion["calidad_contenido"],
